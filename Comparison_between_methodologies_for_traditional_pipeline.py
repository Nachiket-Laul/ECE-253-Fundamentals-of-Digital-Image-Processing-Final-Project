...
# === CONFIGURATION ===
input_folder = r"D:/pregan_onlyclassical/input"
output_folder = r"D:/pregan_onlyclassical/comparisonoutput"
...
def add_label(image, text, color=(0, 255, 0)):
    """Helper to write text on an image"""
    h, w = image.shape[:2]
...
for filename in os.listdir(input_folder):
    if filename.lower().endswith(valid_extensions):
        ...
        # ==========================================
        # 1. GENERATE COUNTERPARTS (DENOISING)
        # ==========================================

        # A. NLM Denoising (The "Standard" for restoration)
        nlm = cv2.fastNlMeansDenoisingColored(img, None, h=6,
hColor=6, templateWindowSize=7, searchWindowSize=21)
        ...
        # ==========================================
        # 2. GENERATE MASKS (Common for Inpainting)
        # ==========================================
        ...
        # ==========================================
        # 3. GENERATE COUNTERPARTS (INPAINTING)
        # ==========================================
        ...
        # ==========================================
        # 4. GENERATE COUNTERPARTS (ENHANCEMENT)
        # ==========================================

        # A. Gamma Correction (Applied to Telea result)
        gamma_val = 1.2
        gamma_corrected = np.array(255 * (telea / 255) ** (1 /
gamma_val), dtype=np.uint8)
        ...
        bilateral_lbl = add_label(bilateral.copy(), "2. Bilateral
Filter", (0, 255, 255))
        gaussian_lbl  = add_label(gaussian.copy(), "3. Gaussian Blur",
(0, 255, 255))
        median_lbl    = add_label(median.copy(), "4. Median Blur", (0,
255, 255))

        # Row 2 Labels
        telea_lbl     = add_label(telea.copy(), "5. Inpaint: Telea",
(0, 255, 0))
        ns_lbl        = add_label(ns_inpaint.copy(), "6. Inpaint: NS",
(0, 255, 255))
        gamma_lbl     = add_label(gamma_corrected.copy(), "7. Enhance:
Gamma", (0, 255, 255))
        clahe_lbl     = add_label(clahe_result.copy(), "8. Enhance:
CLAHE", (0, 255, 0))
